...
- `GET /api/v1/inspection-batches`：按30分钟采集断档自动生成巡检批次
- `GET /api/v1/inspection-batches/:batchNo`：批次温湿度、轨迹和点位气泡数据
...
后端连接 MQTT Broker 后，可以发布一轮 A1～A5 的测试巡检数据：
...
测试数据包含 SLAM 坐标、点位编号和温湿度；A3 默认包含一条湿度越限
记录，用于验证异常批次和橙色数据气泡。
