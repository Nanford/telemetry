...
- Target deployment: Raspberry Pi 4B (Debian Trixie arm64), **Python 3.11**（源码编译，系统自带 3.13 与 cyclonedds 不兼容）
...
Core: `adafruit-circuitpython-dht`, `adafruit-blinka`（提供 board 模块）, `paho-mqtt<2.0`, `pyyaml`, `sqlite-utils`, `requests`, `numpy`, `pyserial`
Go2 SDK2: `cyclonedds==0.10.2`（需先编译 C 库 0.10.2）, `unitree_sdk2_python`（从 GitHub 安装，需 `sed` 修复 b2 导入）
System: `libgpiod-dev`（Debian Trixie 中替代 libgpiod2）, `cmake`, `gcc`, `g++`
